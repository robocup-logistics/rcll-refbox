 ...
 */
void 
WebsocketLogger::build_document(rapidjson::Document* d, const char *component, std::string level, std::string time, std::string message, bool exception) {
 ...
 */
void 
WebsocketLogger::build_document(rapidjson::Document* d, const char *component, std::string level, std::string time, rapidjson::Value& messages, bool exception) {
  rapidjson::Document::AllocatorType& alloc = d->GetAllocator();
...
WebsocketLogger::vlog_debug(const char *component, const char *format, va_list va)
{
  if (log_level <= LL_DEBUG ) {
    ...
    build_document(&d, component, "debug", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  formatter(format, va));
    data_->log_push(d);
...
WebsocketLogger::vlog_info(const char *component, const char *format, va_list va)
{
  if (log_level <= LL_INFO ) {
    ...
    build_document(&d, component, "info", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  formatter(format, va));
    data_->log_push(d);
...
WebsocketLogger::vlog_warn(const char *component, const char *format, va_list va)
{
  if ( log_level <= LL_WARN ) {
    ...
    build_document(&d, component, "warn", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  formatter(format, va));
    data_->log_push(d);
...
WebsocketLogger::vlog_error(const char *component, const char *format, va_list va)
{
  if ( log_level <= LL_ERROR ) {
    ...
    build_document(&d, component, "error", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  formatter(format, va));
    data_->log_push(d);
...
WebsocketLogger::log_debug(const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_DEBUG ) {
    ...
    build_document(&d, component, "debug", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  messages, true);
    data_->log_push(d);
...
WebsocketLogger::log_info(const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_DEBUG ) {
    ...
    build_document(&d, component, "info", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  messages, true);
    data_->log_push(d);
...
WebsocketLogger::log_warn(const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_DEBUG ) {
    ...
    build_document(&d, component, "warn", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)now.tv_usec),  messages, true);
    data_->log_push(d);
...
WebsocketLogger::tlog_debug(struct timeval *t, const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_DEBUG ) {
    ...
    build_document(&d, component, "debug", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  messages, true);
    data_->log_push(d);
...
WebsocketLogger::tlog_info(struct timeval *t, const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_INFO ) {
    ...
    build_document(&d, component, "info", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  messages, true);
    data_->log_push(d);
...
WebsocketLogger::tlog_warn(struct timeval *t, const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_WARN ) {
    ...
    build_document(&d, component, "warn", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  messages, true);
    data_->log_push(d);
...
WebsocketLogger::tlog_error(struct timeval *t, const char *component, fawkes::Exception &e)
{
  if (log_level <= LL_ERROR ) {
    ...
    build_document(&d, component, "error", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  messages, true);
    data_->log_push(d);
...
void
WebsocketLogger::vtlog_debug(struct timeval *t, const char *component, const char *format, va_list va)
{
  if (log_level <= LL_DEBUG ) {
    rapidjson::Document d; 
    d.SetObject();
    ...
    build_document(&d, component, "debug", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  formatter(format, va));
    data_->log_push(d);
...
void
WebsocketLogger::vtlog_info(struct timeval *t, const char *component, const char *format, va_list va)
{
  if (log_level <= LL_INFO ) {
    rapidjson::Document d; 
    d.SetObject();
    ...
    build_document(&d, component, "info", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  formatter(format, va));
    data_->log_push(d);
...
void
WebsocketLogger::vtlog_warn(struct timeval *t, const char *component, const char *format, va_list va)
{
  if ( log_level <= LL_WARN ) {
    rapidjson::Document d; 
    d.SetObject();
    ...
    build_document(&d, component, "warn", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  formatter(format, va));
    data_->log_push(d);
...
void
WebsocketLogger::vtlog_error(struct timeval *t, const char *component, const char *format, va_list va)
{
  if ( log_level <= LL_ERROR ) {
    rapidjson::Document d; 
    d.SetObject();
    ...
    build_document(&d, component, "error", boost::str(fmt_time % now_s->tm_hour % now_s->tm_min % now_s->tm_sec % (long)t->tv_usec),  formatter(format, va));
    data_->log_push(d);
