...
template <class T_CppObject>
inline RefPtr<T_CppObject>::RefPtr() : __cpp_object(0), __ref_count(0), __ref_mutex(0)
{
}
...
: __cpp_object(cpp_object), __ref_count(0), __ref_mutex(0)
{
	if (cpp_object) {
		__ref_count  = new int;
...
: __cpp_object(cpp_object), __ref_count(refcount), __ref_mutex(refmutex)
{
...
: __cpp_object(src.__cpp_object), __ref_count(src.__ref_count), __ref_mutex(src.__ref_mutex)
{
	if (__cpp_object && __ref_count && __ref_mutex) {
		__ref_mutex->lock();
		++(*__ref_count);
