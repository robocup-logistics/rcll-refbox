...
get_argument_iterator(void *cls, enum MHD_ValueKind kind, const char *key, const char *value)
{
	WebRequest *request = static_cast<WebRequest *>(cls);
	if (value == NULL)
		request->set_get_value(key, "");
	else
		request->set_get_value(key, value);
	return MHD_YES;
}
 ...
 */
WebRequest::WebRequest(const char *uri) : pp_(NULL), is_setup_(false), uri_(uri)
{
	reply_size_ = 0;
}
                  ...
                  MHD_Connection *connection)
{
	...
	MHD_get_connection_values(connection, MHD_GET_ARGUMENT_KIND, &get_argument_iterator, this);
	...
	if (headers_.find("X-Forwarded-For") != headers_.end()) {
		...
		default: strncpy(addr_str, "Unknown AF", INET6_ADDRSTRLEN);
		}
