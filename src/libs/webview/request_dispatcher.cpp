 ...
 */
void
WebRequestDispatcher::setup_basic_auth(const char *realm, WebUserVerifier *verifier)
{
 ...
 */
void
WebRequestDispatcher::setup_cors(bool                       allow_all,
                                         ...
                                         void **                session_data)
{
	...
	return rd->process_request(
	  connection, url, method, version, upload_data, upload_data_size, session_data);
}
                                      ...
                                      void **                session_data)
{
	...
	if (!request->is_setup()) {
		...
		if (0 == strcmp(method, MHD_HTTP_METHOD_POST)) {
			request->pp_ = MHD_create_post_processor(connection, 1024, &post_iterator, request);
		}
	...
	if (realm_) {
		...
		if ((user == NULL) || (pass == NULL) || !user_verifier_->verify_user(user, pass)) {
	...
	if (0 == strcmp(method, MHD_HTTP_METHOD_OPTIONS)) {
		...
		if (cors_allow_all_) {
			...
			if (origin != headers.end()) {
				if (std::find(cors_origins_.begin(), cors_origins_.end(), origin->second)
				    != cors_origins_.end()) {
	...
	try {
		...
		if (reply) {
			...
			if (sreply) {
				...
				WebErrorPageReply ereply(WebReply::HTTP_INTERNAL_SERVER_ERROR, "Unknown reply type");
		...
		WebErrorPageReply ereply(WebReply::HTTP_INTERNAL_SERVER_ERROR, "%s", e.what_no_backtrace());
		return queue_static_reply(connection, request, &ereply);
	} catch (std::exception &e) {
		WebErrorPageReply ereply(WebReply::HTTP_INTERNAL_SERVER_ERROR, "%s", e.what());
		return queue_static_reply(connection, request, &ereply);
	}
}
