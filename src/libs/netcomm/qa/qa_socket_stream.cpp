...
{
public:
	StreamServerThread() : Thread("StreamServerThread", Thread::OPMODE_CONTINUOUS)
	{
		i = 0;
		s = new StreamSocket();
		s->bind(1910);
...
{
public:
	StreamClientThread(const char *host) : Thread("StreamClientThread", Thread::OPMODE_CONTINUOUS)
	{
		if (host == NULL) {
			this->host = "127.0.0.1";
		} else {
...
public:
	...
	StreamSocketQAMain(unsigned int mode, const char *host = NULL)
	{
		s = NULL;
		c = NULL;
		if ((mode == MODE_STANDALONE) || (mode == MODE_SERVER)) {
			s = new StreamServerThread();
		}
		if ((mode == MODE_STANDALONE) || (mode == MODE_CLIENT)) {
...
main(int argc, char **argv)
{
	...
	if (argp.has_arg("s") || argp.has_arg("c")) {
		...
		if (argp.has_arg("s")) {
			...
			m = new StreamSocketQAMain(StreamSocketQAMain::MODE_CLIENT, argp.arg("c"));
		}
	} else {
