...
public:
	...
	: Thread("WorldInfoSenderThread", Thread::OPMODE_CONTINUOUS)
	{
		i = 0;
		try {
			t = new WorldInfoTransceiver(
			  WorldInfoTransceiver::MULTICAST, "224.16.0.1", port, "AllemaniACsX", "DoesAnyOneCare", rs);
			t->set_loop(loop);
		} catch (Exception &e) {
			e.print_trace();
			throw;
...
{
public:
	WorldInfoReceiverThread(unsigned short int   port,
	...
	: Thread("WorldInfoReceiverThread", Thread::OPMODE_CONTINUOUS)
	{
		...
		try {
			t = new WorldInfoTransceiver(
			  WorldInfoTransceiver::MULTICAST, "224.16.0.1", port, "AllemaniACs", "WorldInfoQA", rs);
			t->add_handler(this);
		} catch (Exception &e) {
			e.print_trace();
			throw;
	...
	virtual void
	pose_rcvd(const char *from_host, float x, float y, float theta, float *covariance)
	{
		cout << "Pose[" << from_host << "]: (x,y,th)=(" << x << "," << y << "," << theta << "), cov=(";
	...
	virtual void
	velocity_rcvd(const char *from_host, float vel_x, float vel_y, float vel_theta, float *covariance)
	{
		cout << "Velo[" << from_host << "]: (vx,vy,vth)=(" << vel_x << "," << vel_y << "," << vel_theta
		     << ")" << endl;
	}

	virtual void
	ball_pos_rcvd(const char *from_host,
	              bool        visible,
	...
	virtual void
	ball_velocity_rcvd(const char *from_host,
	                   float       vel_x,
	                   float       vel_y,
	                   float       vel_z,
	                   float      *covariance)
	...
	virtual void
	opponent_disapp_rcvd(const char *from_host, unsigned int uid)
	{
