    ...
    (do-for-fact ((?product product)) (eq ?order:id ?product:oid)
      ...
      (do-for-fact ((?m machine)) (and (eq ?m:mtype DS) (eq ?m:team ?team))
         (assert (workpiece (team ?team)
                            ...
                            (holding FALSE)
                            (at-machine ?m:name)
                            (base-color ?product:base-color)
                            (ring-colors ?product:ring-colors)
                            (cap-color ?product:cap-color))))
...
;     (gamestate (phase PRODUCTION) (game-time ?gt))
;     (workpiece-tracking (enabled TRUE))
;     ?wf <- (workpiece (id ?id) (order 0) (team ?r-team) ;noch product rein?
;                       (latest-data TRUE)
;                       (cap-color ?cap-color)
...
;     (test (eq ?order-id ?product-oid))
    
;     (test (or (eq ?cap-color nil)
;               (eq ?cap-color ?product-cap-color)))
;     (test (or (and (eq (length$ ?ring-colors) 0)
;                    (eq (length$ ?product-ring-colors) 0))
;               (and (> (length$ ?ring-colors) 0)
;                    (eq ?ring-colors (subseq$ ?product-ring-colors 1 (length$ ?ring-colors))))))
;     =>
;     (duplicate ?wf (start-time ?gt) (order ?order-id)) ;product or order?
...
;     (gamestate (phase PRODUCTION) (game-time ?gt))
;     (workpiece-tracking (enabled TRUE)) ;#! auskommentieren workpiece tracking
;     ?wf <- (workpiece (id ?id)
;                       (latest-data TRUE)
...
;     (test (or (neq ?base-color ?product-base-color)
;               (and (> (length$ ?ring-colors) 0)
;                    (neq ?ring-colors (subseq$ ?product-ring-colors 1 (length$ ?ring-colors))))
;               (and (neq ?cap-color nil)
;                    (neq ?cap-color ?product-cap-color))))
;     =>
...
;    (order (id ?o1-id) (active TRUE))
;    (product (pid ?pid) (oid ?oid) (complexity ~C0) (ring-colors $?o1-ring-colors))
