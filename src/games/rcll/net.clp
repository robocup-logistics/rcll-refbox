...
(deffunction net-create-StorageInfo (?team-color ?mps-ss)
	(bind ?si (pb-create "llsf_msgs.StorageInfo"))
	(pb-set-field ?si "team_color" ?team-color)
	(pb-set-field ?si "machine" ?mps-ss)
	(do-for-all-facts ((?ssf machine-ss-shelf-slot))
	                  ...
	                  (pb-add-list ?si "shelf_slot_info" ?ssf-pb)
	)
	(return ?si)
)

(defrule net-send-StorageInfo
	(time $?now)
	?f <- (signal (type storage-info) (time $?t&:(timeout ?now ?t ?*STORAGE-INFO-PERIOD*)) (seq ?seq))
	(gamestate (cont-time ?ctime) (phase PRODUCTION))
	(machine (name ?ss-cyan) (mtype SS) (team ?cyan&CYAN))
	(machine (name ?ss-magenta) (mtype SS) (team ?magenta&MAGENTA))
	(network-peer (group ?cyan) (id ?peer-id-cyan))
	(network-peer (group ?magenta) (id ?peer-id-magenta))
	=>
	(modify ?f (time ?now) (seq (+ ?seq 1)))
	(bind ?si-cyan (net-create-StorageInfo ?cyan ?ss-cyan))
	(bind ?si-magenta (net-create-StorageInfo ?magenta ?ss-magenta))
	(pb-broadcast ?peer-id-cyan ?si-cyan)
	(pb-broadcast ?peer-id-magenta ?si-magenta)
)
