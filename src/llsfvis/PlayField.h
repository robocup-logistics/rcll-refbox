...
private:
	void draw_machine(const Cairo::RefPtr<Cairo::Context> &cr, const Machine &machine);
	void draw_puck(const Cairo::RefPtr<Cairo::Context> &cr, const Puck &puck);
	void draw_machine_t(const Cairo::RefPtr<Cairo::Context> &cr,
	                    int                                  size,
	                    int                                  wPos,
	                    int                                  hPos,
	                    Machine::Orientation                 orientation);
	void draw_text(const Cairo::RefPtr<Cairo::Context> &cr, int x, int y, std::string text);
