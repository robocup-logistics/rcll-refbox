...
class DataExtractor:
  def __init__(
    ...
    for x in all_reports:
      ...
      total_points_cyan = {
          ...
          "score": "total",
          "team": "CYAN",
          "total-points": x["total-points"][0],
      }
      total_points_magenta = {
          "game": game,
          "score": "total",
          "team": "MAGENTA",
          "total-points": x["total-points"][1],
      }
      ...
      for p in x["points"]:
        ...
        if p["reason"].find("Delivered item") != -1:
          ...
          complexity = next(o for o in x["orders"] if o["id"] == numbers[0])[
              "complexity"
          ]
          delivery = {
              "game": game,
      ...
      machine_history = sorted(
          x["machine-history"],
          key=lambda k: k["game-time"])
          ...
          for end_entry in machine_history[idx:]:
            if end_entry["name"] == start_entry["name"]:
              ...
              if (
                ...
                self.raw_data[Data.MPS_OUTPUT_READY].append(
                    flatten(proc)
                )
                found = True
                break
  ...
  def cross_game_evaluate(self):
    ...
    for data in Data:
      ...
      splits = set()
      for g in games:
        tmp_data, tmp_keys, tmp_splits = self.structure_data(
            data,
            filter_func=lambda a: [x for x in a if x["game.report-name"] == g],
        )
  ...
  def box_plot_axis(self, ax, d):
    ...
    ax.legend(
        handles=[x["boxes"][0] for x in plots],
        labels=list(split_vals),
        loc="upper right",
        bbox_to_anchor=(1, 1 + 0.2 * len(split_vals)),
    )
  ...
  def table_axis(self, tabax, d):
    ...
    for k, v in self.cross_game_data[d].items():
      for s, data in v.items():
        column_data = list(
            np.percentile(list(data.values()), [0, 25, 50, 75, 100])
        )
  ...
  def export_cross_game_data(self):
    with open("output.csv", "wt") as file:
      ...
      writer.writerow(
          map(
              lambda e: e.text, soup.find_all(
                  "td", {
                      "class": "snapshot-td2-cp"})))
      # write body row
      writer.writerow(
          map(lambda e: e.text, soup.find_all("td", {"class": "snapshot-td2"}))
      )
      ...
      for k, v in self.cross_game_data[d].items():
        for s, data in v.items():
          ...
          quantiles = np.quantile(
              list_data, np.array([0.00, 0.25, 0.50, 0.75, 1.00])
          )
      plots = list()
      for i, s in enumerate(split_vals):
        plots.append(
            ax.boxplot(
                values[s],
                positions=[
                    len(split_vals) * y + i for y in range(len(values[s]))
                ],  # , notch=True
                widths=0.35,
                patch_artist=True,
            )
  ...
  def transform_point_reason(self, reason, orders):
    if (
        reason.find("eliver") != -1
        or reason.find("Mounted cap") != -1
        or reason.find("ring") != -1
    ):
      numbers = [int(s) for s in reason.split() if s.isdigit()]
      complexity = next(x for x in orders if x["id"] == numbers[0])[
          "complexity"]
      return complexity
  ...
  def structure_data(self, data_type, filter_func=lambda a: a):
    ...
    split_vals = (
        set()
        .union(d["game.CYAN"] for d in filtered_data[data_type])
        .union(d["game.MAGENTA"] for d in filtered_data[data_type])
    )
    if "" in split_vals:
  ...
  def bar_diagram(self, data, x_label, y_label, file_name):
    ...
    for i, s in enumerate(split_vals):
      ...
      bar = ax.bar(x + (i + 0.5) * width - total_bar_width /
                   2, plot_data, width, label=s)
