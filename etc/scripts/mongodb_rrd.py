...
class MongoRRD(object):
    def __init__(self, graph_clear=False, mongodb_host=None, mongodb_port=None, mongodb_name="llsfrb"):
        self.quit = False
    ...
    def init_rrd(self):
        ...
        self.create_rrd("memory.rrd", "DS:resident:GAUGE:30:0:U", "DS:virtual:GAUGE:30:0:U", "DS:mapped:GAUGE:30:0:U")
...
def main(argv):
    parser = OptionParser()
    parser.add_option(
        "--mongodb-host", dest="mongodb_host", help="Hostname of MongoDB", metavar="HOST", default="localhost"
    )
    parser.add_option(
        "--mongodb-port", dest="mongodb_port", help="Hostname of MongoDB", type="int", metavar="PORT", default=27017
    )
    parser.add_option(
        "--mongodb-name",
        dest="mongodb_name",
    ...
    parser.add_option(
        "--graph-clear", dest="graph_clear", default=False, action="store_true", help="Remove existing RRD files."
    )
